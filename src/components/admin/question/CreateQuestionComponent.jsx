...
import {adminCreateQuestion} from "../../../api/admin/AdminQuestion";

const CreateQuestionComponent = () => {
    const {id} = useParams();
    const [searchParams, setSearchParms] = useSearchParams();
    const [selectAnswers, setSelectAnswers] = useState([]);
    const [inputAnswers, setInputAnswers] = useState([{id: 1, text: "", title: "Answer1"}, {
    ...
    const addNewAnswer = () => {
        ...
        setInputAnswers((current) => [...current, {id: index, text: "", title: `Answer${index}`, length: {length}},]);
    ...
    const handleSubmit = () => {
        if (handleValidateForm()) {
            ...
            const object = {
                vocabularyId: id, question: question, rightAnswer: indexRightAnswer, answers: arrayAnswer,
            };
            adminCreateQuestion(object).then((response) => {
                navigate(-1);
            }).catch((errors) => {
                console.log(errors);
            });
            ...
            <div className="card-body">
                ...
                <form className="row g-3">
                    <div className="col-12">
                        ...
                        <CKEditor
                            editor={Editor}
                            onChange={(event, editor) => handleChangeQuestion("question", editor.getData())}
                            type="text"
                    ...
                    <div className="col-12">
                        <div className="form-floating">
                            ...
                            >
                                {selectAnswers.map((item) => (<option value={item.id - 1}>
                    ...
                    {inputAnswers.map((item) => (<InputAnswer
                        id={item.id}
                        text={item.text}
                        title={item.title}
